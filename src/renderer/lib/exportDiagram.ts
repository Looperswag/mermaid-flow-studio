...
import { extractSvgDimensions, type DiagramTheme } from './renderMermaid';

const paperColor: Record<DiagramTheme, string> = {
  light: '#f6efe4',
  dark: '#141b19',
};
...
export async function buildExportBlob(input: {
  svg: string;
  format: ExportFormat;
  quality: ExportQuality;
  background: ExportBackground;
  /** Active diagram theme; a paper background follows it so dark diagrams stay readable. */
  theme?: DiagramTheme;
}) {
  ...
  const background =
    input.format === 'jpg' || input.background === 'paper'
      ? paperColor[input.theme ?? 'light']
      : 'transparent';
